...
GBDT 的正则化是"外挂的"，靠限制树深、学习率间接控制。XGBoost 把 $\gamma T + \frac{1}{2}\lambda\sum w_j^2$ 直接写进目标函数，分裂增益公式里天然含惩罚，不值得分裂时模型自动停止。

**3. 叶子权重有闭式解**

引入二阶信息后，每个叶子的最优输出可直接算出 $w_j^* = -G_j/(H_j+\lambda)$。GBDT 没有这个，叶子值通常取简单均值。

一句话：**GBDT 是函数空间的一阶梯度下降，XGBoost 是带正则的二阶牛顿法**。正则项是外在表现，二阶信息才是本质升级。
...
- **XGBoost**：把 $f_t(x_i)$ 视为增量 $\Delta x$，对损失做关于这个增量的泰勒展开，自然出现二次项，也就有了二阶导 $h_i$。
